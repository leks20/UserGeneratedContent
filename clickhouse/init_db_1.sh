...
(
    ...
    progress Float32,
    event_time DateTime
) Engine=ReplicatedMergeTree('/clickhouse/tables/shard1/movie_watch_events', 'replica_1') PARTITION BY toYYYYMMDD(event_time) ORDER BY (user_id, movie_id, event_time);
